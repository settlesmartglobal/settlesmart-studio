...
export function CommerceForm({ companies, categories }: { companies: Company[]; categories?: Category[] }) {
  ...
  return (
    <form className="grid gap-4 md:grid-cols-2" onSubmit={async (event) => {
      ...
      {categories && <label className="grid gap-1 text-sm font-medium">Dietary<select name="dietaryClassification" defaultValue="" className="rounded-md border border-slate-200 px-3 py-2"><option value="">Not applicable</option><option value="VEG">VEG</option><option value="NON_VEG">NON_VEG</option></select></label>}
