...
export function ProductEditForm({ product, categories }: { product: ClientProduct; categories: Array<{ id: string; name: string }> }) {
  ...
  const [error, setError] = useState("");
  const [imagePath, setImagePath] = useState(String(product.imagePath ?? ""));
...
export function FeedbackForm({ orderNumber, token }: { orderNumber: string; token: string }) {
  ...
  }}><h2 className="font-semibold">Feedback</h2><input type="hidden" name="rating" value={rating} /><div className="flex gap-1" role="radiogroup" aria-label="Rating">{[1, 2, 3, 4, 5].map((value) => <button key={value} type="button" role="radio" aria-checked={rating === value} aria-label={`${value} star - ${ratingLabels[value]}`} onClick={() => setRating(value)} className={`min-h-11 min-w-11 rounded-md text-3xl leading-none ${value <= rating ? "text-amber-500" : "text-slate-300"}`}>{value <= rating ? "★" : "☆"}</button>)}</div><p className="text-sm font-semibold text-slate-700">{ratingLabels[rating]}</p><textarea name="comment" placeholder="Comment optional" className="rounded-md border border-slate-200 px-3 py-2" /><button className="rounded-md bg-slate-950 px-4 py-2 text-sm font-semibold text-white">Submit feedback</button>{message && <p className="text-sm text-emerald-700">{message}</p>}{error && <p className="text-sm text-red-600">{error}</p>}</form>;
}
...
export function CommerceSettingsForm({ company }: { company: CommerceSettingsCompany }) {
  ...
  return <form className="grid gap-3 text-sm md:grid-cols-2" onSubmit={async (event) => {
    ...
    <input name="country" defaultValue={company.country ?? ""} placeholder="Country" className="rounded-md border border-slate-200 px-3 py-2" />
    <input name="currencyCode" defaultValue={company.currencyCode ?? String(settings.currency ?? "AED")} placeholder="Currency code" className="rounded-md border border-slate-200 px-3 py-2 uppercase" />
    <input name="orderPrefix" defaultValue={company.orderPrefix ?? "SS"} placeholder="Order prefix" className="rounded-md border border-slate-200 px-3 py-2 uppercase" />
    <input name="region" defaultValue={company.region ?? ""} placeholder="Region / State / Emirate" className="rounded-md border border-slate-200 px-3 py-2" />
    <input name="city" defaultValue={company.city ?? ""} placeholder="City" className="rounded-md border border-slate-200 px-3 py-2" />
    <input name="postalCode" defaultValue={company.postalCode ?? ""} placeholder="Postal code optional" className="rounded-md border border-slate-200 px-3 py-2" />
    ...
    <input type="hidden" name="currency" value={company.currencyCode ?? String(settings.currency ?? "AED")} />
