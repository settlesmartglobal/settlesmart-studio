...
export default async function MediaAssetPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!asset) notFound();
  return <AppShell title={asset.title}><Panel><p className="text-sm text-slate-500">{asset.company.name} · {asset.assetType} · {asset.category} · {asset.approvalStatus}</p>{asset.mimeType.startsWith("image/") && <AssetImage src={asset.filePath} alt={asset.title} className="mt-5 aspect-video max-h-[520px] w-full rounded-md object-cover" />}{asset.mimeType.startsWith("video/") && <video src={normalizePublicAssetPath(asset.filePath)} controls className="mt-5 max-h-[520px] rounded-md" />}<a href={normalizePublicAssetPath(asset.filePath)} download className="mt-5 inline-flex rounded-md bg-slate-950 px-4 py-2 text-sm font-semibold text-white">Download</a></Panel></AppShell>;
}
