...
export default async function DeliveryPage() {
  ...
  return <AppShell title="Delivery"><div className="grid gap-5 lg:grid-cols-[420px_1fr]"><Panel><DeliveryForm companies={companies.map(toClientCompanySummary)} /></Panel><Panel>{zones.map((z) => <div key={z.id} className="border-b border-slate-100 py-3"><b>{z.name}</b><span className="text-sm text-slate-500"> · {z.company.name} · {String(z.radiusKm)} km · {formatCommerceMoney(z.deliveryCharge, z.company.currencyCode)}</span></div>)}</Panel></div></AppShell>;
}
