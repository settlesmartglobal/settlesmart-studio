    ...
    <main className="min-h-screen bg-[#f7f8fb] text-slate-950">
      <div className="mx-auto flex min-h-screen max-w-7xl flex-col px-4 py-4 sm:px-6">
        ...
        <section className="mt-5 flex-1 rounded-lg border border-slate-200 bg-white/75 p-4 shadow-sm">
          ...
          {company && active === "overview" && <div className="space-y-5"><div className="grid gap-4 md:grid-cols-4"><Metric label="Orders today" value={String(todayOrders.length)} /><Metric label="Revenue today" value={`AED ${formatMoney(revenueToday)}`} /><Metric label="Average order value" value={`AED ${formatMoney(todayOrders.length ? revenueToday / todayOrders.length : 0)}`} /><Metric label="Pending" value={String(orders.filter((order) => order.status === "PENDING").length)} /></div><div className="grid gap-4 md:grid-cols-4"><Metric label="Preparing" value={String(orders.filter((order) => order.status === "PREPARING").length)} /><Metric label="Ready" value={String(orders.filter((order) => order.status === "READY").length)} /><Metric label="Out for delivery" value={String(orders.filter((order) => order.status === "OUT_FOR_DELIVERY").length)} /><Metric label="Completed" value={String(completed.length)} /></div><LiveOperations orders={orders} /><div className="grid gap-4 lg:grid-cols-3"><Card title="Business readiness"><p className="text-3xl font-semibold">{readiness?.percent === 100 ? "100% Ready" : `${readiness?.percent ?? 0}%`}</p><p className={`mt-1 text-sm font-semibold ${readiness?.status === "READY TO ACCEPT ORDERS" ? "text-emerald-700" : "text-amber-700"}`}>{readiness?.status === "READY TO ACCEPT ORDERS" ? "Ready to Accept Orders" : "Setup Incomplete"}</p><div className="mt-3 grid gap-2 text-sm">{readiness?.items.map((item) => <div key={`${item.group}-${item.label}`} className="rounded-md bg-slate-50 px-3 py-2"><div className="flex justify-between gap-3"><span>{item.label}</span><b className={item.ok ? "text-emerald-700" : "text-amber-700"}>{item.ok ? "READY" : "NEEDED"}</b></div>{!item.ok && <p className="mt-1 text-xs text-slate-500">{item.missing}</p>}</div>)}</div></Card><Card title="Quick actions"><div className="grid gap-2 text-sm">{[["Add Product", "/commerce/products/new"], ["Import Catalogue", "/commerce/products"], ["Create Category", "/commerce/categories"], ["View Incoming Orders", "/commerce?section=orders&status=PENDING"], ["Open Kitchen", "/commerce?section=kitchen"], ["Add Rider", "/commerce?section=delivery"], ["Review Notifications", "/commerce?section=settings"], ["Preview Customer Store", publicUrl]].map(([action, href]) => <Link key={action} href={href || "/commerce"} className="rounded-md border border-slate-200 px-3 py-2 font-semibold">{action}</Link>)}</div></Card><Card title="QR & Store Link"><p className="mb-3 font-semibold">{company.commerceSettings?.displayName ?? company.name}</p>{publicUrl ? <><QrCodeBox url={publicUrl} /><div className="mt-3 flex flex-wrap gap-2 text-sm"><a href={whatsappStoreShareUrl(company.commerceSettings?.displayName ?? company.name, publicUrl)} className="rounded-md border border-slate-200 px-3 py-2 font-semibold">Share on WhatsApp</a><Link href={publicUrl} className="rounded-md border border-slate-200 px-3 py-2 font-semibold">Preview Customer Store</Link></div></> : <p className="text-sm text-slate-500">Set an ordering slug first.</p>}</Card></div><Card title="Recent orders"><RecentOrdersCompact orders={orders.slice(0, 5)} /><Link href="/commerce?section=orders&status=ALL" className="mt-4 inline-flex rounded-md border border-slate-200 px-3 py-2 text-sm font-semibold">View All Orders</Link></Card></div>}
...
function LiveOperations({ orders }: { orders: CommerceOrders }) {
  const items = [
    { label: "Pending", status: "PENDING", href: "/commerce?section=orders&status=PENDING", action: "View Orders" },
    ...
    { label: "Out for Delivery", status: "OUT_FOR_DELIVERY", href: "/commerce?section=delivery", action: "Open Delivery" },
  ];
  return <Card title="Live operations"><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{items.map((item) => <Link key={item.status} href={item.href} className="rounded-md border border-slate-200 bg-slate-50 p-4"><p className="text-sm text-slate-500">{item.label}</p><p className="mt-2 text-2xl font-semibold">{orders.filter((order) => order.status === item.status).length}</p><p className="mt-3 text-sm font-semibold text-emerald-700">{item.action}</p></Link>)}</div></Card>;
}
