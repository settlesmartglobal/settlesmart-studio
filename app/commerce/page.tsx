...
function ServiceabilityPanel({ serviceability, compact = false }: { serviceability: ReturnType<typeof orderServiceability>; compact?: boolean }) {
  ...
  return <div className={`mt-3 rounded-md border p-3 text-sm ${serviceability.isWithinDeliveryRadius === false ? "border-red-200 bg-red-50 text-red-800" : "border-emerald-200 bg-emerald-50 text-emerald-800"}`}><p className="font-semibold">{unavailable ? "Delivery distance unavailable" : serviceability.isWithinDeliveryRadius ? "Within Delivery Area" : "Outside configured delivery area"}</p>{!compact && <div className="mt-2 grid gap-1 text-slate-700"><p>Distance from restaurant: {formatKm(serviceability.distanceKm)}</p><p>Delivery radius: {formatKm(serviceability.deliveryRadiusKm)}</p></div>}{compact && !unavailable && <p className="mt-1 text-xs">Distance {formatKm(serviceability.distanceKm)} · Radius {formatKm(serviceability.deliveryRadiusKm)}</p>}</div>;
}
